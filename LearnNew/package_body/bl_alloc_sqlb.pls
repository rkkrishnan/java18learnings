CREATE OR REPLACE PACKAGE BODY BL_ALLOC_SQL AS
------------------------------------------------------------------------------------
FUNCTION ALLOC_PO(O_error_message         IN OUT VARCHAR2,
   ...
   cursor C_GET_ORD_CHILD_ITEMS is
      select i.item,
             (SUM(o.unit_cost*o.qty_ordered) / SUM(o.qty_ordered)) unit_cost,
             SUM(o.qty_received) qty_received,
             SUM(o.qty_ordered) qty_ordered
        from item_master i,ordloc o
       where o.order_no = I_order_no
         and o.item              = i.item
         and (i.item_parent      = L_item
          or  i.item_grandparent = L_item)
       group by i.item ;
   ...
   cursor C_GET_UNIT_COST(L_item ITEM_MASTER.ITEM%TYPE) is
      select (SUM(unit_cost*qty_ordered) / SUM(qty_ordered)) unit_cost
        from ordloc
       where order_no = I_order_no
         and item     = L_item;

   cursor C_GET_CO_INVC is
...
BEGIN
   if I_alloc_basis_uom is not NULL then
      ...
      FOR C_rec in C_GET_ITEMS LOOP
         ...
         ---
         FOR L_rec in C_GET_CO_INVC LOOP
      ...
      FOR C_rec in C_GET_ITEMS LOOP
         ...
         ---
         if L_item_level = L_tran_level then
            SQL_LIB.SET_MARK('OPEN','C_GET_UNIT_COST','ORDLOC','ITEM: '||L_item);
            open C_GET_UNIT_COST(L_item);
            SQL_LIB.SET_MARK('FETCH','C_GET_UNIT_COST','ORDLOC','ITEM: '||L_item);
            fetch C_GET_UNIT_COST into L_unit_cost;
            SQL_LIB.SET_MARK('CLOSE','C_GET_UNIT_COST','ORDLOC','ITEM: '||L_item);
            ...
            if L_child_items_exist = 'Y' then
               ...
               FOR L_rec in C_GET_TRAN_CHILD_ITEMS LOOP
                  ...
                  ---
                  SQL_LIB.SET_MARK('OPEN','C_GET_UNIT_COST','ORDLOC','ITEM: '||L_item);
                  open C_GET_UNIT_COST(L_child_item);
                  SQL_LIB.SET_MARK('FETCH','C_GET_UNIT_COST','ORDLOC','ITEM: '||L_item);
                  fetch C_GET_UNIT_COST into L_temp_unit_cost;
                  SQL_LIB.SET_MARK('CLOSE','C_GET_UNIT_COST','ORDLOC','ITEM: '||L_item);
                  close C_GET_UNIT_COST;
                  ---
               ...
               FOR L_rec in C_GET_ORD_CHILD_ITEMS LOOP
                  L_child_item       := L_rec.item;
                  L_item_rec_qty     := NVL(L_rec.qty_received, L_rec.qty_ordered);
                  L_temp_unit_cost   := L_rec.unit_cost;
                  ---
                  L_unit_cost := L_unit_cost + (L_temp_unit_cost * L_item_rec_qty);
               END LOOP;
            end if;
...
FUNCTION ALLOC_BL_AWB(O_error_message         IN OUT VARCHAR2,
                      I_obligation_key        IN     OBLIGATION.OBLIGATION_KEY%TYPE,
                      I_obligation_level      IN     OBLIGATION.OBLIGATION_LEVEL%TYPE,
                      I_bl_awb_id             IN     TRANSPORTATION.BL_AWB_ID%TYPE,
                      I_vessel_id             IN     TRANSPORTATION.VESSEL_ID%TYPE,
   ...
   L_program            VARCHAR2(64)             := 'BL_ALLOC_SQL.ALLOC_BL_AWB';
   L_child_items_exist  VARCHAR2(1)              := 'N';
   L_order_no           ORDHEAD.ORDER_NO%TYPE;
   L_item               ITEM_MASTER.ITEM%TYPE;
   L_child_item         ITEM_MASTER.ITEM%TYPE;
   ...
   L_bl_awb_total       OBLIGATION_COMP.QTY%TYPE            := 0;
   ...
   L_packitem_qty        ORDLOC.QTY_RECEIVED%TYPE;
   ...
   cursor C_GET_SUPP_CTRY(L_item  ITEM_MASTER.ITEM%TYPE) is
      select o.supplier,
       ...
       where o.order_no = l.order_no
         ...
         and l.item     = s.item
         and l.item     = L_item
    group by o.supplier,s.origin_country_id,s.item ;
...
BEGIN
   ...
   FOR C_rec in C_GET_ORD_ITEM LOOP
      ...
      ---
      if L_item_level = L_tran_level then
         SQL_LIB.SET_MARK('OPEN','C_GET_SUPP_CTRY','ORDHEAD,ORDLOC','Order No: '||to_char(L_order_no)||
                                                                    ' Item: '||L_item);
         open C_GET_SUPP_CTRY(L_item);
         SQL_LIB.SET_MARK('FETCH','C_GET_SUPP_CTRY','ORDHEAD,ORDLOC','Order No: '||to_char(L_order_no)||
                                                                     ' Item: '||L_item);
         fetch C_GET_SUPP_CTRY into L_supplier,
                                    L_origin_country_id,
                                    L_unit_cost;
         SQL_LIB.SET_MARK('CLOSE','C_GET_SUPP_CTRY','ORDHEAD,ORDLOC','Order No: '||to_char(L_order_no)||
                                                                     ' Item: '||L_item);
      ...
      if nvl(L_pack_type,'N') = 'B' then
         ...
         FOR C_rec in C_GET_PACKITEMS LOOP
            ...
            ---
            L_pack_qty := L_pack_qty + (L_qty * L_packitem_qty);
         END LOOP;
      ...
      ---
      L_bl_awb_total := L_bl_awb_total + L_item_rec_qty;
      ---
   ...
   FOR C_rec in C_GET_ORD LOOP
      ...
      ---
      if L_bl_awb_total = 0 or L_total_cost = 0 then
         L_qty := 0;
         L_amt := 0;
      else
         L_qty := I_qty * (L_ord_item_total / L_bl_awb_total);
         ---
         if I_alloc_basis_uom is not NULL then
            L_amt := I_amt_prim * (L_ord_item_total / L_bl_awb_total);
         else
