CREATE OR REPLACE PACKAGE BODY DIFFGRPHDR_XML AS

--------------------------------------------------------------------------------
FUNCTION DELETE_DIFFGRPHDR(O_status     OUT    VARCHAR2,
                           O_text       OUT    VARCHAR2,
                           I_record     IN     DIFF_GROUP_HEAD%ROWTYPE,
                           root         IN OUT xmldom.DOMElement)
RETURN BOOLEAN;
--------------------------------------------------------------------------------
FUNCTION ADD_UPDATE_DIFFGRPHDR(O_status          OUT    VARCHAR2,
                               O_text            OUT    VARCHAR2,
                               I_record          IN     DIFF_GROUP_HEAD%ROWTYPE,
                               root              IN OUT xmldom.DOMElement)
RETURN BOOLEAN;
...
BEGIN
   if I_action_type = 'D' then
      ...
      ---
      if not DELETE_DIFFGRPHDR(O_status,
                               O_text,
                               I_record,
                               root) then
         return FALSE;
      ...
      L_document_type := 'DiffGrpHdrDesc';
      ---
      if not API_LIBRARY.CREATE_MESSAGE(O_status,
                                        O_text,
                                        root,
      ...
      ---
      if not ADD_UPDATE_DIFFGRPHDR(O_status,
                                   O_text,
                                   I_record,
                                   root) then
         return FALSE;
...
EXCEPTION
   when OTHERS then
      API_LIBRARY.HANDLE_ERRORS(O_status,
                                O_text,
                                API_LIBRARY.FATAL_ERROR,
                                'DIFFGRPHDR_XML.DELETE_DIFFGRPHDR');
      return FALSE;

END DELETE_DIFFGRPHDR;
--------------------------------------------------------------------------------
FUNCTION ADD_UPDATE_DIFFGRPHDR(O_status          OUT    VARCHAR2,
                               O_text            OUT    VARCHAR2,
                               I_record          IN     DIFF_GROUP_HEAD%ROWTYPE,
...
BEGIN
   ...
   rib_xml.addElement( root, 'diff_group_id', I_record.diff_group_id);
   rib_xml.addElement( root, 'diff_group_type', I_record.diff_type);
   rib_xml.addElement( root, 'diff_group_desc', I_record.diff_group_desc);
...
EXCEPTION
   when OTHERS then
      API_LIBRARY.HANDLE_ERRORS(O_status,
                                O_text,
                                API_LIBRARY.FATAL_ERROR,
                                'DIFFGRPHDR_XML.ADD_UPDATE_DIFFGRPHDR');
      return FALSE;

END ADD_UPDATE_DIFFGRPHDR;
--------------------------------------------------------------------------------
END DIFFGRPHDR_XML;
/
